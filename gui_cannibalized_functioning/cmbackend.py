...
class readFTDIFifoThread(QThread):
    ...
    def run(self):
        ...
        while self._running:
            ...
            ''' commented out because no 198 is passed from discovery board
            if data == False:
                pass
            elif data[1] == 198:
                dataQueue.put(data)
...
class streamAdcThread(QThread):
    ...
    def run(self):
        ...
        samples = 0
        t_0 = time.time()
        data_flag = 0 #flag to say which part of data to write
        ...
        while self._running:
            ...
            if not timeout:
                ...
                self.values = [data[i] for i in range(0,67)]
                # out.append(self.values)
                out.append([self.values[i] for i in self.plotCh])
                self.msbs = [self.values[i] for i in range(0,64)]
                hdout.append(self.msbs)
                data_point['time'] = data_time
                data_point.append()
...
class CMWorker(QThread):
    ...
    def startStream(self):
        if not self.cp2130Handle:
            return
        self._regWr(Reg.start, 0x0020)

    def stopStream(self):
        if not self.cp2130Handle:
            return
        self._regWr(Reg.stop, 0x0010)
