...
import datetime
import libusb
libusb.config(LIBUSB = r"C:\Users\Utente\Documents\BIOWOLF\libusb-1.0")
...
def cp2130_libusb_set_usb_config(handle):
    buf = c_ubyte * 10
    control_buf_out = buf(0x00, 0x00, 0x00, 0x00, 0x00,
                          0x00, 0x00, 0x00, 0x00, 0x80)
    usbTimeout = 5000

    error_code = libusb.libusb_control_transfer(
        handle, 0x40, 0x61, 0xA5F1, 0x000A, control_buf_out, sizeof(control_buf_out), usbTimeout)
...
def set_gpio_mode_and_level(handle, number, mode, level):
    ...
    error_code = libusb.libusb_control_transfer(
        handle, 0x40, 0x23, 0x0000, 0x0000, control_buf_out, sizeof(control_buf_out), usbTimeout)
    if error_code != sizeof(control_buf_out):
        print('Error in bulk transfer')
        return False
        return True
...
def cp2130_libusb_read(handle, datalen):
    ...
    error_code = libusb.libusb_bulk_transfer(handle, 0x02, read_command_buf, sizeof(
        read_command_buf), byref(bytesWritten), usbTimeout)
    if error_code:
    ...
    error_code = libusb.libusb_bulk_transfer(handle, 0x81, read_input_buf, sizeof(
        read_input_buf), byref(bytesRead), usbTimeout)
    if error_code:
...
def cp2130_libusb_set_spi_word(handle, ch, word):
    ...
    error_code = libusb.libusb_control_transfer(
        handle, 0x40, 0x31, 0x0000, 0x0000, control_buf_out, sizeof(control_buf_out), usbTimeout)
    if error_code != sizeof(control_buf_out):
        print('Error in bulk transfer')
        return False
...
def cp2130_libusb_get_spi_word(handle):
    ...
    error_code = libusb.libusb_control_transfer(
        handle, 0xC0, 0x30, 0x0000, 0x0000, control_buf_out, sizeof(control_buf_out), usbTimeout)
    if error_code != sizeof(control_buf_out):
        print('Error in bulk transfer')
        return False
...
context = libusb.libusb_context_p()
deviceList = libusb.libusb_device_p_p()
deviceCount = 0
deviceDescriptor = libusb.libusb_device_descriptor()
device = libusb.libusb_device_p()
cp2130Handle = libusb.libusb_device_handle_p()
kernelAttached = 0
dev_list = []

if libusb.libusb_init(byref(context)) != 0:
    print('Could not initialize libusb!')


deviceCount = libusb.libusb_get_device_list(context, byref(deviceList))
if deviceCount <= 0:
    print('No devices found!')
for i in range(0, deviceCount):
    if libusb.libusb_get_device_descriptor(deviceList[i], byref(deviceDescriptor)) == 0:
...
if libusb.libusb_open(device, byref(cp2130Handle)) != 0:
...
if libusb.libusb_kernel_driver_active(cp2130Handle, 0) != 0:
    libusb.libusb_detach_kernel_driver(cp2130Handle, 0)
    self.kernelAttached = 1
    print('return')

if libusb.libusb_claim_interface(cp2130Handle, 0) != 0:
...
released = libusb.libusb_release_interface(cp2130Handle, 0)
if released:
