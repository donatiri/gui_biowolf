...
# this function always writes 1 byte
def cp2130_libusb_write_one(handle, value): # value should 1 byte to write
    buf = c_ubyte * 9
...
def cp2130_libusb_set_spi_word(handle):
    buf = c_ubyte * 2
    control_buf_out = buf(0x00, 0x09)  #0x09 if 6Mhz, 0x08 if 12Mhz
    usbTimeout = 5000
    ...
    def run(self):
        ...
        while self._running:
            ...
            if data == False:
                ...
                dataQueue.put(data)
                timeQueue.put(time.time() - t_0)
                CMWorker().ackStream() # put CM into streaming mode for both NMs
    ...
    def run(self):
        ...
        while self._running:
            ...
            if not timeout:
                ...
                '''   consider always the case with no CRC and make no operations on data
                ...
                success += 1
                data_point = self.dataTable.row
                data_point['out'] = [data[i] for i in list(range(0, datalen - 3, 2))]
                self.values = [data[i] for i in range(0,33)]
                out.append(self.values)
                out.append([self.values[i] for i in self.plotCh])
                self.msbs = [self.values[i] for i in range(0,33)]
                hdout.append(self.msbs)
                data_point['time'] = data_time
                data_point.append()
                '''
...
class CMWorker(QThread):
    ...
    def _regWr(self, reg):
        cp2130_libusb_write_one(CMWorker.cp2130Handle, reg.value)
